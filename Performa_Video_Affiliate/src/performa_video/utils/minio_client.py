...
def get_sheet_watermarks(minio_client: Minio, bucket: str, watermark_path: str, sheet_registry: dict | None = None) -> tuple[dict, list]:
    ...
    ({"sheets": [...]}); a legacy GLOBAL format ({"last_processed_date": ...})
    is also accepted and translated to creds-keyed rows via sheet_registry.
    """
    try:
    ...
    # === FAILSAFE START: legacy GLOBAL format ({"last_processed_date": ...}) ===
    # Format lama menyimpan SATU tanggal global, bukan tabel per sheet.
    # Diterjemahkan jadi satu baris per creds di registry supaya migrasi
    # ke format baru tidak me-reprocess seluruh data.
    # REMOVE this block after the next successful run.
    if "sheets" not in data:
        global_date = str(data.get("last_processed_date", "")).strip()[:10]
        records = []
        if global_date:
            for sheet_name, creds in (sheet_registry or {}).items():
                records.append({
                    "creds": creds,
                    "sheet_name": sheet_name,
                    "last_processed_date": global_date,
